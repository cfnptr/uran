	...
	void* argument)
{
	...
	UserInterface ui = createUserInterfaceInstance(
		logger, threadPool, window, packReader);
	...
	logMessage(logger, INFO_LOG_LEVEL, "Initialized.");
	showWindow(window);
	return engine;
}
void destroyEngine(Engine engine)
{
	...
	terminateGraphics();
	destroyThreadPool(engine->threadPool);

	if (engine->logger)
	{
		logMessage(engine->logger,
			INFO_LOG_LEVEL, "Terminated.");
...
ThreadPool getEngineThreadPool(Engine engine)
{
	assert(engine);
	return engine->threadPool;
}
...
// TODO: remove uran scripts/
//  make windows-pack script
//  use include in the vulkan shaders
